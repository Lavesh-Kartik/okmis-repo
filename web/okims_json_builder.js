...
function openBuilderModal(node) {
  ...
  toolbar.className = "okims-modal-toolbar";

  const title = document.createElement("div");
  title.className = "okims-modal-title";
  title.textContent = "Okims_JSON_Builder";

  const actions = document.createElement("div");
  actions.className = "okims-modal-actions";

  const applyBtn = document.createElement("button");
  applyBtn.className = "primary";
  applyBtn.textContent = "Confirm & Send JSON Prompt";

  const copyBtn = document.createElement("button");
  copyBtn.textContent = "Copy JSON Prompt";

  const closeWrap = document.createElement("div");
  closeWrap.className = "okims-modal-spacer";
  ...
  toolbar.append(title, actions, closeWrap);
  ...
  iframe.addEventListener("load", () => {
    ...
    try {
      ...
      console.warn("[Okims_JSON_Builder] Unable to preload JSON into iframe", e);
    }
  });
  ...
  function readJSONFromFrame() {
    try {
      ...
      console.error("[Okims_JSON_Builder] Unable to read JSON from iframe", e);
  ...
  applyBtn.addEventListener("click", () => {
    ...
    if (!json.trim()) {
      toast("Builder JSON is empty", "warn");
      return;
    }
    setWidgetValue(node, json);
    toast("Sent to ComfyUI node", "ok");
  });

  copyBtn.addEventListener("click", () => copyText(readJSONFromFrame()));
}
